...
void setup () {
  ...
  // remove battery, upload code including the line below, re-upload with line below commented out, replace battery
  rtc.adjust(DateTime(F(__DATE__), F(__TIME__))); // sets RTC clock based on system clock
  //rtc.adjust(DateTime(2014, 1, 21, 23, 30, 0)); // YYYY, M, D, H, M, S
  
  FastLED.addLeds<APA102, DATA_PIN, CLOCK_PIN, BGR>(leds, NUM_LEDS); // Red & Blue are flipped
}

void loop () { // Define MAIN program(s)
  DateTime now = rtc.now(); // poll DS1307
  h = now.hour();
  m = now.minute();
  s = now.second();
  ...
  serial_print(ch,h_c,cl,h,m,s); // debug printout -- state bool, hour count, clock switch, h:m:s
//  clock_scheduler(cl,h,m,s); // run clock scheduler
  clock4(h,m,s);

  delay(10 * 1000); // update 1 time per 10 seconds (formerly 10 times)
}
...
void serial_print(int ch,int h_c,int cl,int h,int m,int s) {
  Serial.print(ch);
  Serial.print(' ');
  Serial.print(h_c);
  Serial.print(' ');
